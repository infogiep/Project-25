...
std::string Chat::signUp(int connection)
{
	...
	{
		netConnect.sendMessage(connection, "Congratulations, you have successfully registered.");
		return login;
	}
	else
	{
...
std::string Chat::entrance(int connection)
{
	...
	{
		netConnect.sendMessage(connection, "Login completed successfully.");
		return login;
	}
	else
	{
		netConnect.sendMessage(connection, "There is no such username/password. You may not be registered yet.");
		return "error";
	}
}
